...
class ProductsController {
  ...
  static async findOne(req, res) {
    try {
      const { error, data } = await ProductsService.findOne(req.params.id);
      if (error) {
        return res.status(400).send(data);
      }
      res.status(201).send(data);
  ...
  static async update(req, res) {
    try {
      const { error, data } = await ProductsService.update(
        req.params.id,
        req.body
      );
      if (error) {
  ...
  static async deleteOne(req, res) {
    try {
      const { error, data } = await ProductsService.deleteOne(req.params.id);
      if (error) {
        return res.status(400).send(data);
      }
      res.status(201).send(data);
